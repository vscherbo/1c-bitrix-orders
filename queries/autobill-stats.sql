...
FROM arc_energo.bx_order bo 
where 
bo.billcreated in (2,6,7,10) -- частичный
and dt_insert > now()- '1 day'::INTERVAL;
\t off
SELECT bx_order_no AS "Заказ", descr AS "Описание"
FROM aub_log 
    WHERE bx_order_no IN
        (SELECT bx_order_no
            FROM aub_log
            where res_code IN (2,6,7,10) AND mod_id = '-1' AND dt_insert > now()- '1 day'::INTERVAL)
